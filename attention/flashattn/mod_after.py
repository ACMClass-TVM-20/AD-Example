...
class Module:
    ...
    def backward_op(dO: T.Buffer((6, 32, 512, 128), "float16"), Q: T.Buffer((6, 32, 512, 128), "float16"), K: T.Buffer((6, 32, 512, 128), "float16"), V: T.Buffer((6, 32, 512, 128), "float16"), O: T.Buffer((6, 32, 512, 128), "float16"), L: T.Buffer((6, 32, 512), "float16"), dQ: T.Buffer((6, 32, 512, 128), "float16"), dK: T.Buffer((6, 32, 512, 128), "float16"), dV: T.Buffer((6, 32, 512, 128), "float16")):
        ...
        for b_nh_fused in T.thread_binding(192, thread="blockIdx.x"):
            for seq in range(512):
                for head_dim in T.thread_binding(128, thread="threadIdx.x"):
                    with T.block("compute_D"):
                        ab = T.axis.spatial(6, b_nh_fused // 32)
                        anh = T.axis.spatial(32, b_nh_fused % 32)
                        aseq, ahead_dim = T.axis.remap("SR", [seq, head_dim])
        ...
        for b in T.thread_binding(6, thread="blockIdx.z"):
            for nh in T.thread_binding(32, thread="blockIdx.y"):
                for j in T.thread_binding(64, thread="blockIdx.x"):
                    with T.block("outer_compute"):
                        ...
                        for v0 in range(8):
                            for v1 in T.thread_binding(128, thread="threadIdx.x"):
                        ...
                        for i in range(64):
                            with T.block("inner_compute"):
                                ...
                                for v0_v1_fused_0 in range(1):
                                    for v0_v1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                        for v2 in range(128):
                                            with T.block("compute_Sij"):
                                                av0 = T.axis.spatial(8, (v0_v1_fused_0 * 128 + v0_v1_fused_1) // 8)
                                                av1 = T.axis.spatial(8, (v0_v1_fused_0 * 128 + v0_v1_fused_1) % 8)
                                                av2 = T.axis.reduce(128, v2)
                                                T.where(v0_v1_fused_0 * 128 + v0_v1_fused_1 < 64)
                                ...
                                for v0, v1 in T.grid(8, 8):
                                    for v2 in T.thread_binding(128, thread="threadIdx.x"):
        ...
        for b_nh_v0_v1_fused_0 in T.thread_binding(98304, thread="blockIdx.x"):
            for b_nh_v0_v1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                with T.block("store_dQ"):
                    ab = T.axis.spatial(6, (b_nh_v0_v1_fused_0 * 128 + b_nh_v0_v1_fused_1) // 2097152)
                    anh = T.axis.spatial(32, (b_nh_v0_v1_fused_0 * 128 + b_nh_v0_v1_fused_1) % 2097152 // 65536)
                    av0 = T.axis.spatial(512, (b_nh_v0_v1_fused_0 * 128 + b_nh_v0_v1_fused_1) % 65536 // 128)
                    av1 = T.axis.spatial(128, (b_nh_v0_v1_fused_0 * 128 + b_nh_v0_v1_fused_1) % 128)
    ...
    def forward_op(Q: T.Buffer((6, 32, 512, 128), "float16"), K: T.Buffer((6, 32, 512, 128), "float16"), V: T.Buffer((6, 32, 512, 128), "float16"), O: T.Buffer((6, 32, 512, 128), "float16"), L: T.Buffer((6, 32, 512), "float16")):
        ...
        Sij = T.alloc_buffer((8, 8), scope="shared")
        Pij = T.alloc_buffer((8, 8), scope="shared")
        for b_nh_i_fused in T.thread_binding(12288, thread="blockIdx.x"):
            with T.block("outer_compute"):
                ab = T.axis.spatial(6, b_nh_i_fused // 2048)
                anh = T.axis.spatial(32, b_nh_i_fused % 2048 // 64)
                ai = T.axis.spatial(64, b_nh_i_fused % 64)
                ...
                for v0, v1_0 in T.grid(8, 1):
                    for v1_1 in T.thread_binding(16, thread="threadIdx.y"):
                        for v1_2 in T.thread_binding(8, thread="threadIdx.x"):
                            with T.block("load_Qi"):
                                av0 = T.axis.spatial(8, v0)
                                av1 = T.axis.spatial(128, v1_0 * 128 + v1_1 * 8 + v1_2)
                                T.reads(Q[ab, anh, ai * 8 + av0, av1])
                                T.writes(Qi[av0, av1])
                                Qi[av0, av1] = Q[ab, anh, ai * 8 + av0, av1]
                for v0, v1_0 in T.grid(8, 1):
                ...
                for j in range(64):
                    with T.block("inner_compute"):
                            ...
                            for v1_1 in T.thread_binding(16, thread="threadIdx.y"):
                                for v1_2 in T.thread_binding(8, thread="threadIdx.x"):
                                    with T.block("load_Kj"):
                                        ...
                                        T.writes(Kj[av0, av1])
                                        Kj[av0, av1] = K[ab, anh, aj * 8 + av0, av1]
                        for v0, v1, v2_0 in T.grid(8, 8, 1):
                            for v2_1 in T.thread_binding(16, thread="threadIdx.y"):
                                for v2_2 in T.thread_binding(8, thread="threadIdx.x"):
                                    with T.block("compute_Sij"):
                                        av0, av1 = T.axis.remap("SS", [v0, v1])
                                        av2 = T.axis.reduce(128, v2_0 * 128 + v2_1 * 8 + v2_2)
                                        T.reads(Qi[av0, av2], Kj[av1, av2])
                                        T.writes(Sij[av0, av1])
                                        with T.init():
                                            Sij[av0, av1] = T.float32(0)
                        ...
                        for v0_v1_fused_0 in range(1):
                            for v0_v1_fused_1 in T.thread_binding(16, thread="threadIdx.y"):
                                for v0_v1_fused_2 in T.thread_binding(8, thread="threadIdx.x"):
                                    with T.block("compute_Pij"):
                                        av0 = T.axis.spatial(8, (v0_v1_fused_0 * 128 + v0_v1_fused_1 * 8 + v0_v1_fused_2) // 8)
                                        av1 = T.axis.spatial(8, (v0_v1_fused_0 * 128 + v0_v1_fused_1 * 8 + v0_v1_fused_2) % 8)
                                        T.where((v0_v1_fused_0 * 16 + v0_v1_fused_1) * 8 + v0_v1_fused_2 < 64)
                ...
                for v0_0 in range(1):
                    for v0_1 in T.thread_binding(16, thread="threadIdx.y"):
                        for v0_2 in T.thread_binding(8, thread="threadIdx.x"):
                            with T.block("store_L"):
                                av0 = T.axis.spatial(8, v0_0 * 128 + v0_1 * 8 + v0_2)
                                T.where((v0_0 * 16 + v0_1) * 8 + v0_2 < 8)
