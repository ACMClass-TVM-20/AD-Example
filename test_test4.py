...
class InputModule:
    @R.function
    def main(
        x: R.Tensor((2, 3, 28, 28), "float32"), y: R.Tensor((3, 4, 3, 3), "float32")
    ):
        with R.dataflow():
            z1 = R.nn.conv2d_transpose(x, y)
            R.output(z1)
        return z1
...
patterns = [
        ...
        # ("cudnn.add", is_op("relax.add")(wildcard(), wildcard())),
]
...
# Prepare inputs.
import torch
t0 = torch.rand(2, 3, 28, 28, dtype=torch.float32)
t1 = torch.rand(3, 4, 3, 3, dtype=torch.float32)

expected = torch.nn.functional.conv_transpose2d(t0, t1)
...
# Run VM.
out = vm["main"](tvm.nd.array(t0.numpy()), tvm.nd.array(t1.numpy()))

tvm.testing.assert_allclose(out.numpy(), expected.numpy(), rtol=1e-6, atol=1e-6)
