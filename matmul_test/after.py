...
class Module:
    ...
    def fused_fused_relax_permute_dims_relax_matmul_cast(B: T.Buffer((T.int64(4096), T.int64(4096)), "float16"), A: T.Buffer((T.int64(1), T.int64(128), T.int64(4096)), "float16"), var_compute_intermediate: T.Buffer((T.int64(1), T.int64(128), T.int64(4096)), "float16")):
        T.func_attr({"tir.is_scheduled": 1, "tir.noalias": T.bool(True)})
        # with T.block("root"):
        A_reindex_shared_dyn = T.alloc_buffer((T.int64(1), T.int64(128), T.int64(8), T.int64(2), T.int64(16), T.int64(16)), "float16", scope="shared.dyn")
        A_reindex_shared_dyn_wmma_matrix_a = T.alloc_buffer((T.int64(1), T.int64(128), T.int64(8), T.int64(2), T.int64(16), T.int64(16)), "float16", scope="wmma.matrix_a")
        var_T_transpose_intermediate_reindex_shared_dyn = T.alloc_buffer((T.int64(32), T.int64(128), T.int64(8), T.int64(2), T.int64(16), T.int64(16)), "float16", scope="shared.dyn")
        var_T_transpose_intermediate_reindex_shared_dyn_wmma_matrix_b = T.alloc_buffer((T.int64(32), T.int64(128), T.int64(8), T.int64(2), T.int64(16), T.int64(16)), "float16", scope="wmma.matrix_b")
        p_output0_intermediate_reindex_shared_dyn = T.alloc_buffer((T.int64(1), T.int64(32), T.int64(8), T.int64(8), T.int64(16), T.int64(16)), scope="shared.dyn")
        p_output0_intermediate_reindex_shared_dyn_wmma_accumulator = T.alloc_buffer((T.int64(1), T.int64(32), T.int64(8), T.int64(8), T.int64(16), T.int64(16)), scope="wmma.accumulator")
        for ax0 in T.thread_binding(T.int64(1), thread="blockIdx.z"):
            for ax1_0_0 in T.thread_binding(T.int64(1), thread="blockIdx.y"):
                for ax2_0_0 in T.thread_binding(T.int64(32), thread="blockIdx.x"):
                    for ax1_0_1_init in T.thread_binding(T.int64(2), thread="threadIdx.z"):
                        for ax2_0_1_init in T.thread_binding(T.int64(2), thread="threadIdx.y"):
                            for ax1_0_2_init, ax2_0_2_init in T.grid(T.int64(4), T.int64(4)):
                                with T.block("matmul_o_init"):
                                    v0_o = T.axis.spatial(T.int64(1), ax0)
                                    v1_o = T.axis.spatial(T.int64(8), ax1_0_0 * T.int64(8) + ax1_0_1_init * T.int64(4) + ax1_0_2_init)
                                    v2_o = T.axis.spatial(T.int64(256), ax2_0_0 * T.int64(8) + ax2_0_1_init * T.int64(4) + ax2_0_2_init)
                                    T.reads()
                    ...
                    for ax3_0_0 in T.unroll(T.int64(128)):
                                    ...
                                    for ax0_ax1_fused_3 in T.thread_binding(T.int64(32), thread="threadIdx.x"):
                                        for ax0_ax1_fused_4 in T.vectorized(T.int64(8)):
                                            with T.block("A_reindex_shared.dyn"):
                                                ...
                                                T.reads(A[v0, v1, v2])
                                                T.writes(A_reindex_shared_dyn[T.int64(0), v2 // T.int64(32), v1 // T.int64(16), v2 % T.int64(32) // T.int64(16), v1 % T.int64(16), v2 % T.int64(16)])
                                                T.block_attr({"buffer_dim_align": [[0, 4, 16, 8]]})
                                                A_reindex_shared_dyn[T.int64(0), v2 // T.int64(32), v1 // T.int64(16), v2 % T.int64(32) // T.int64(16), v1 % T.int64(16), v2 % T.int64(16)] = A[v0, v1, v2]
                        for ax0_ax1_fused_0 in range(T.int64(4)):
                            for ax0_ax1_fused_1 in T.thread_binding(T.int64(2), thread="threadIdx.z"):
                                for ax0_ax1_fused_2 in T.thread_binding(T.int64(2), thread="threadIdx.y"):
                                    for ax0_ax1_fused_3 in T.thread_binding(T.int64(32), thread="threadIdx.x"):
                                        for ax0_ax1_fused_4 in T.vectorized(T.int64(8)):
                                            with T.block("var_T_transpose_intermediate_reindex_shared.dyn"):
                                                v0 = T.axis.spatial(T.int64(1), T.int64(0))
                                                v1 = T.axis.spatial(T.int64(4096), ax2_0_0 * T.int64(128) + (ax0_ax1_fused_0 * T.int64(1024) + ax0_ax1_fused_1 * T.int64(512) + ax0_ax1_fused_2 * T.int64(256) + ax0_ax1_fused_3 * T.int64(8) + ax0_ax1_fused_4) // T.int64(32))
                                                v2 = T.axis.spatial(T.int64(4096), ax3_0_0 * T.int64(32) + (ax0_ax1_fused_0 * T.int64(1024) + ax0_ax1_fused_1 * T.int64(512) + ax0_ax1_fused_2 * T.int64(256) + ax0_ax1_fused_3 * T.int64(8) + ax0_ax1_fused_4) % T.int64(32))
                                                T.reads(B[v1, v2])
                        ...
                        for ax1_0_1 in T.thread_binding(T.int64(2), thread="threadIdx.z"):
                            for ax2_0_1 in T.thread_binding(T.int64(2), thread="threadIdx.y"):
                                for ax3_0_1 in range(T.int64(2)):
                                    for ax0_1 in range(T.int64(4)):
                                        with T.block("A_reindex_shared.dyn_wmma.matrix_a_o"):
                                            ...
                                            v2_o = T.axis.spatial(T.int64(8), ax1_0_1 * T.int64(4) + ax0_1)
                                    ...
                                    for ax0_1 in range(T.int64(4)):
                                        with T.block("var_T_transpose_intermediate_reindex_shared.dyn_wmma.matrix_b_o"):
                                            v0_o, v1_o = T.axis.remap("SS", [ax2_0_0, ax3_0_0])
                                            v2_o = T.axis.spatial(T.int64(8), ax2_0_1 * T.int64(4) + ax0_1)
                                            v3_o = T.axis.spatial(T.int64(2), ax3_0_1)
                                            v4_o = T.axis.spatial(T.int64(1), T.int64(0))
                                            v5_o = T.axis.spatial(T.int64(1), T.int64(0))
                                    ...
                                    for ax1_0_2, ax2_0_2 in T.grid(T.int64(4), T.int64(4)):
                                        with T.block("matmul_o_update"):
                                            v0_o = T.axis.spatial(T.int64(1), ax0)
                                            v1_o = T.axis.spatial(T.int64(8), ax1_0_0 * T.int64(8) + ax1_0_1 * T.int64(4) + ax1_0_2)
                                            v2_o = T.axis.spatial(T.int64(256), ax2_0_0 * T.int64(8) + ax2_0_1 * T.int64(4) + ax2_0_2)
                    ...
                    for ax0_1, ax1 in T.grid(T.int64(1), T.int64(1)):
                            ...
                            for ax3_0 in T.thread_binding(T.int64(2), thread="threadIdx.y"):
                                for ax2_1, ax3_1 in T.grid(T.int64(4), T.int64(4)):
                                    with T.block("p_output0_intermediate_reindex_shared.dyn_wmma.accumulator_o"):
                                        v0_o = T.axis.spatial(T.int64(1), ax1_0_0 + ax0_1)
                                        v1_o = T.axis.spatial(T.int64(32), ax2_0_0 + ax1)
                                ...
                                for ax0_ax1_fused_3 in T.thread_binding(T.int64(32), thread="threadIdx.x"):
                                    for ax0_ax1_fused_4 in T.vectorized(T.int64(8)):
                                        with T.block("p_output0_intermediate_reindex_shared.dyn"):
                                            v0 = T.axis.spatial(T.int64(1), T.int64(0))
                                            v1 = T.axis.spatial(T.int64(128), (ax0_ax1_fused_0 * T.int64(1024) + ax0_ax1_fused_1 * T.int64(512) + ax0_ax1_fused_2 * T.int64(256) + ax0_ax1_fused_3 * T.int64(8) + ax0_ax1_fused_4) // T.int64(128))
                                            v2 = T.axis.spatial(T.int64(4096), ax2_0_0 * T.int64(128) + (ax0_ax1_fused_0 * T.int64(1024) + ax0_ax1_fused_1 * T.int64(512) + ax0_ax1_fused_2 * T.int64(256) + ax0_ax1_fused_3 * T.int64(8) + ax0_ax1_fused_4) % T.int64(128))
                                            T.reads(p_output0_intermediate_reindex_shared_dyn[T.int64(0), v2 // T.int64(128), v1 // T.int64(16), v2 % T.int64(128) // T.int64(16), v1 % T.int64(16), v2 % T.int64(16)])
                                            T.writes(var_compute_intermediate[T.int64(0), v1, v2])
                                            var_compute_intermediate[T.int64(0), v1, v2] = T.Cast("float16", p_output0_intermediate_reindex_shared_dyn[T.int64(0), v2 // T.int64(128), v1 // T.int64(16), v2 % T.int64(128) // T.int64(16), v1 % T.int64(16), v2 % T.int64(16)])

    @R.function
    def main(A: R.Tensor((1, 128, 4096), dtype="float16"), B: R.Tensor((4096, 4096), dtype="float16")) -> R.Tensor((1, 128, 4096), dtype="float16"):
        cls = Module
        with R.dataflow():
            gv = R.call_tir(cls.fused_fused_relax_permute_dims_relax_matmul_cast, (B, A), out_sinfo=R.Tensor((1, 128, 4096), dtype="float16"))
            R.output(gv)
        return gv
