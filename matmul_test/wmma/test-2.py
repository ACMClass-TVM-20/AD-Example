"""Test: fp16 mixed precision matmul + epilogue (mul2 + add)
- With unroll: 97.31235904865969
- Without unroll: 140.21669080779947
"""
import os
import sys
import tvm
...
if not reload:
    ...
    class Module:
        @R.function
        def main(
            A: R.Tensor(shape_1, dtype), B: R.Tensor(shape_2, dtype), C: R.Tensor(shape_3, dtype)
        ):
            with R.dataflow():
                lv1 = R.permute_dims(B)
                lv2 = R.matmul(A, lv1, out_dtype=fallback_dtype)
                lv3 = R.astype(lv2, dtype)
                lv4 = lv3 * R.const(2, dtype)
                gv = R.add(lv4, C)
                R.output(gv)
            return gv
    ...
    # build
    func_name = "fused_fused_relax_permute_dims_relax_matmul_cast_multiply_add"
...
# generate inputs
np_inputs = [
    np.random.normal(size=size).astype(dtype) for size in [shape_1, shape_2, shape_3, shape_3]
]
torch_inputs = [torch.tensor(x).to("cuda") for x in np_inputs[:3]]
tvm_inputs = [tvm.nd.array(x, dev) for x in np_inputs]

# Step 1. check correctness
ex(tvm_inputs[1], tvm_inputs[0], tvm_inputs[2], tvm_inputs[3])
torch.backends.cuda.matmul.allow_fp16_reduced_precision_reduction = False
torch_res = torch_inputs[0] @ torch_inputs[1].T * 2 + torch_inputs[2]
assert np.allclose(torch_res.detach().cpu().numpy(), tvm_inputs[3].numpy(), atol=1e-3, rtol=1e-3)
print("<correctness check done>")

# Step 2. check performance
eval = ex.time_evaluator(ex.entry_name, dev, 10, 10)
report = eval(tvm_inputs[1], tvm_inputs[0], tvm_inputs[2], tvm_inputs[3])
print(report)

op_time = report.mean
tflops = (batch * shape_m * shape_n * shape_k * 2 + batch * shape_m * shape_n * 2) / op_time / 1e12
