...
std::string readFileIntoRawString(const std::string& filePath) {
...
const std::string CUSTOM_KERNEL = readFileIntoRawString("build.metal");
// static const char *CUSTOM_KERNEL = .c_str();
...
torch::Tensor& dispatchSoftShrinkKernel(const torch::Tensor& input, torch::Tensor& output,
                                        float lambda) {
  @autoreleasepool {
    ...
    id<MTLLibrary> customKernelLibrary =
        [device newLibraryWithSource:[NSString stringWithUTF8String:CUSTOM_KERNEL.c_str()]
                             options:nil
                               error:&error];
    TORCH_CHECK(customKernelLibrary, "Failed to to create custom kernel library, error: ",
                error.localizedDescription.UTF8String);

    std::string kernel_name = std::string("softshrink_kernel_") +
                              (input.scalar_type() == torch::kFloat ? "float" : "half");
    id<MTLFunction> customSoftShrinkFunction = [customKernelLibrary
        newFunctionWithName:[NSString stringWithUTF8String:kernel_name.c_str()]];
    TORCH_CHECK(customSoftShrinkFunction, "Failed to create function state object for ",
                kernel_name.c_str());

    // Create a compute pipeline state object for the soft shrink kernel.
    id<MTLComputePipelineState> softShrinkPSO =
        [device newComputePipelineStateWithFunction:customSoftShrinkFunction error:&error];
    ...
    // Get a reference to the dispatch queue for the MPS stream, which encodes the synchronization
    // with the CPU.
    ...
    dispatch_sync(serialQueue, ^() {
      ...
      [computeEncoder setBuffer:getMTLBufferStorage(input) offset:0 atIndex:0];
      [computeEncoder setBuffer:getMTLBufferStorage(output) offset:0 atIndex:1];
      ...
      // MTLSize threadsPerThreadgroup = MTLSizeMake(numThreads, 1, 1);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) { m.def("mps_softshrink", &mps_softshrink); }
