...
def clean(x):
    return urllib.parse.unquote(str(x)).strip()
...
class DataPreprocessor:
    ...
    def parse_file(self, infile, chardict, labeldict):
        ...
        for line in fin:                
            entity, label = map(clean, line.rstrip().split('\t')[:2])
            # print entity
            ent = list(map(lambda c:chardict[c], list(entity)))
            lab = list(map(lambda l:labeldict[l] if l in labeldict else 0, label.split(',')))
