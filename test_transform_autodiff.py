...
def test_tuple():
    ...
    class Before:
        @R.function
        def main(x: Tensor((1, 10), "float32"),
                 y: Tensor((1, 10), "float32"),
                 z: Tensor((1, 10), "float32"),
                 w: Tensor((1, 10), "float32"),
                 v: Tensor((1, 10), "float32")):
            with R.dataflow():
                t = (x, y, z)
                lv1 = relax.add(t[0], t[1])
                loss = relax.nn.softmax_cross_entropy(lv1, w)
                R.output(loss)
            return loss
    
    After = relax.transform.SimpleAD("main", "loss")(Before)
    After.show()
