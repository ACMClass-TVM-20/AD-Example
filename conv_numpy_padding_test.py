...
# nchw x oikk
# padding = 1
# output shape: N, O, H, W
# forward
def conv2d(input, weight, output):
	N, C, H, W = input.shape
	O, I, K, _ = weight.shape
	P = 1
	pad_tmp = np.zeros((N, C, H+P*2, W+P*2))
	for n, c, h, w in grid(N, C, H, W):
		pad_tmp[n, c, h, w] = input[n, c, h - P, w - P] if h >= P and h <= H + P and w >= P and w <= W + P else 0
	for n, o, h1, w1, i, k1, k2 in grid(N, O, H - K + 1 + P * 2, W - K + 1 + P * 2, I, K, K):
...
def conv2d_adj(input, weight, output, output_adj, input_adj, weight_adj):
	...
	for n, o, h1, w1, i, k1, k2 in grid(N, O, H - K + 1 + P * 2, W - K + 1 + P * 2, I, K, K):
		# initing output doesn't influence result
...
output_t = F.conv2d(input_t, weight_t, padding=1)
