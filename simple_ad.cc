 ...
 public:
  ...
  void VisitBinding_(const VarBindingNode* binding) override {
    ...
    if (const auto* node = binding->value.as<TupleNode>()) {
      UpdateExprMap(GetRef<Tuple>(node), adjoint_var);
    }
    ...
    // b = a[0]
      ...
      UpdateExprMap(GetRef<TupleGetItem>(node), adjoint_var);
    }
    // case 3: assign
    // a = b
    // b_adjoint_expr += a_adjoint_var
    else if (const auto* node = binding->value.as<VarNode>()) {
      UpdateExprMap(GetRef<Var>(node), adjoint_var);
    }
    // case 4: call
  ...
  Expr DoAdd(const Expr& src1, const Expr& src2) {
    ...
    if (const auto* node1 = src1.as<TupleNode>()) {
      if (const auto* node2 = src2.as<TupleNode>()) {
        ...
        Array<Expr> result;
        for (int i = 0; i < node1->fields.size(); ++i) {
          result.push_back(DoAdd(node1->fields[i], node2->fields[i]));
        }
        return Tuple(result);
      }
      else {
        LOG(FATAL) << "type not match" << std::endl;
      }
    }
    else {
  ...
  void UpdateExprMap(const Expr& base, const Expr& increment) {
    if (const auto* node = base.as<VarNode>()) {
      ...
      for (int i = 0; i < node->fields.size(); ++i) {
        const Expr& now_field = TupleGetItem(increment, i);
        UpdateExprMap(node->fields[i], now_field);
      }
    }
    else if (const auto* node = base.as<TupleGetItemNode>()) {
      Array<int> indices;
      while (node->tuple.as<TupleGetItemNode>()) {
        indices.push_back(node->index);
        node = node->tuple.as<TupleGetItemNode>();
      }
      ...
      if (adjoint_expr_map.count(v) == 0) {
        // use shape expr as default adjoint expr
        // TODO(chaofan): better solution
        adjoint_expr_map.Set(v, Tuple(node->tuple->shape().as<TupleNode>()->fields));
      }
      auto* cur = adjoint_expr_map[v].as<TupleNode>();
      for (int i = 0; i < indices.size(); ++i) {
        int index = indices[i];
        ICHECK(cur->fields.size() < index) << "Index error when handling GetItem AD" << std::endl;
        ICHECK(cur->fields[index].as<TupleNode>()) << "Type error when handling GetItem AD" << std::endl;
        if (i < indices.size() - 1) {
          cur = cur->fields[index].as<TupleNode>();
        }
        else {
          cur->fields.Set(index, (relay::Expr)DoAdd(cur->fields[index], increment));
        }
      }
    }
    else {
